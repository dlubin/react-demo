        ...
        return (
            <section className="flex flex-col flex-nowrap gap-y-2">
                <ul className="list-none pl-0 border-b border-slate-400 pb-2 mb-0">
                    { cart.map((id, index) =>(
                        <li>{ items.games[id].title }</li>
                    ))}
                </ul>
                <span>Subtotal: ${ cart.reduce((acc, current) => acc + parseFloat(items.games[current].price), 0).toFixed(2) }</span>
                <button className="rounded p-2 bg-yellow-600 text-slate-900" onClick={() => setShowToast(true)}>Checkout</button>
                <ToastContainer position="bottom-center" className="position-fixed">
                    <Toast onClose={() => setShowToast(false)} show={showToast} bg="info" className="!text-gray-950 !text-md">
                        <Toast.Header className="justify-between">This is just a demo!</Toast.Header>
